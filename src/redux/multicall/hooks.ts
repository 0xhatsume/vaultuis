...
export function useMultipleContractSingleDataMultipleInputs(
    addresses: (string | undefined)[],
    contractInterface: Interface,
    methodName: string,
    callInputs?: OptionalMethodInputs[],
    ...
    const calls = useMemo(() => {
        const validCallInputs =
            !callInputs ||
            (callInputs &&
                callInputs?.length > 0 &&
                callInputs?.length === addresses.length &&
                callInputs.every((inputs) => isValidMethodArgs(inputs)));
        if (fragment && addresses && addresses.length > 0 && validCallInputs) {
            return addresses.map<Call | undefined>((address, i) =>
                address && callInputs[i]
                    ? {
                          address,
    ...
    return useMemo(
        ...
        [fragment, results, latestBlockNumber, contractInterface],
    );
}

export function useMultipleContractMultipleDataMultipleInputs(
    addresses: (string | undefined)[],
    ...
    const fragments = useMemo(
        () =>
            contractInterfaces.map((contractInterface, i) =>
                contractInterface.getFunction(methodNames[i]),
            ),
        // eslint-disable-next-line react-hooks/exhaustive-deps
        [contractInterfaces, methodNames.join(",")],
    );
    ...
    const calls = useMemo(() => {
        ...
        if (fragments && addresses && addresses.length > 0 && validCallInputs) {
            return addresses.map<Call | undefined>((address, i) =>
                address
                    ? {
                          address,
                          callData: contractInterfaces[i].encodeFunctionData(
                              fragments[i],
                              callInputs?.[i],
                          ),
                          ...(gasRequired ? { gasRequired } : {}),
        ...
        return [];
    }, [addresses, callInputs, contractInterfaces, fragments, gasRequired]);
